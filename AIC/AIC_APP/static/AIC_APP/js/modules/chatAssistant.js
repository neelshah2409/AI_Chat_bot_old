$(document).ready(()=>{
    ...
    $.ajax({
        ...
        success: function (data) {
            $("#welcomeMessage").val(data.greet);
            $("#sorryMessage").val(data.error);
        },
        error: function(){
    ...
    const validateSettings = (welcome,sorry)=>{
        if(welcome==""){
            $("#welcomeMessage").addClass("is-invalid")
            $("#welcomeMessage").placeholder("Welcome Message is Required")
            return false;
        }
        if(sorry==""){
            $("#sorryMessage").addClass("is-invalid")
            $("#sorryMessage").placeholder("Welcome Message is Required")
            return false;
        }
        return true;
    }
    $("#settingsForm").on("submit",(e)=>{
        ...
        if(validateSettings(welcome,sorry))
        {
            $.ajax({
                type: "get",
                url: "/setgreeterrmsg",
                method: "POST",
                data:{"greet":welcome, "sorry":sorry},
                success: function (data) {
                    console.log(data);
                    $("#changesStatus").html("Changes Saved Successfully")
                    $("#changesStatus").addClass("text-success");
                },
                error: function(){
                    $("#changesStatus").html("Failed To Save Changes")
                    $("#changesStatus").addClass("text-danget");
                }
            });
            setTimeout(()=>{
                $("#changesStatus").html("")
